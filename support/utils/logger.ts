 ...
 *
 * Levels (severity order, highest priority first):
 * - ERROR — actionable failures
 * - WARN  — recoverable oddities
 * - INFO  — user-relevant progress milestones (one per public POM method)
 * - HTTP  — request/response traces (verbose)
 * - DEBUG — verbose diagnostics (paths, intermediate values, step traces)
 *
 * Default effective level (when LOG_LEVEL is unset):
 * - CI (NODE_ENV=CI or truthy CI env): INFO
 * - Local runs: DEBUG
 *
 * Override anytime: LOG_LEVEL=error|warn|info|http|debug (case-insensitive).
 */
export enum LogLevel {
  ...
  export class Logger {
    ...
    static resolveLevelFromEnv(): LogLevel {
      const raw = process.env.LOG_LEVEL?.trim();
      if (raw) {
        const normalized = raw.toUpperCase();
        if (VALID_LOG_LEVELS.has(normalized)) {
    ...
    error(message: string, ...args: any[]): void {
      ...
      if (args.length > 0 && args[0] instanceof Error) {
        ...
        console.error(`${Colors.red}${Colors.bright}   ↳ Error details:${Colors.reset} ${error.message}`);
        if (error.stack) {
          console.error(`${Colors.red}${Colors.dim}   ↳ Stack trace:${Colors.reset}`);
    ...
    http(message: string, ...args: any[]): void {
      if (!Logger.shouldLog(LogLevel.HTTP)) {
        return;
      }
      console.info(this.formatMessage(LogLevel.HTTP, message));
      if (args.length > 0) {
        console.info(`${Colors.magenta}${Colors.dim}   ↳ Additional data:${Colors.reset}`, ...args);
      }
    }
    ...
    // Utility method for creating section dividers in logs
    section(title: string): void {
      if (!Logger.shouldLog(LogLevel.DEBUG)) {
        return;
      }
      const divider = `${Colors.gray}${'─'.repeat(50)}${Colors.reset}`;
      console.log(`\n${divider}`);
    ...
    success(message: string): void {
      ...
      const contextStr = this.context ? `${Colors.dim}[${this.context}]${Colors.reset}` : '';
      const levelStr = `${successColor}${Colors.bright}PASS ${Colors.reset}`;
